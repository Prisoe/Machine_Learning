...
def bintime_to_timeofday(input_data):
    """
    Converts time into time-of-day categories. Handles both DataFrames and single values.

    Parameters:
        input_data (int or pd.Series or pd.DataFrame): The hour or a column/DataFrame containing the hour.

    Returns:
        str or pd.Series: Time-of-day category (e.g., "Morning") for single values,
                          or a DataFrame/Series with the "TIME_BIN" column added for DataFrames.
    """
    ...
    # Handle single value for API use
    if isinstance(input_data, int):
        return bin_time(input_data)

    # Handle DataFrame or Series for project use
    if isinstance(input_data, pd.Series):
        return input_data.apply(bin_time)
    elif isinstance(input_data, pd.DataFrame) and "TIME" in input_data.columns:
        input_data["TIME_BIN"] = input_data["TIME"].apply(bin_time)
        return input_data
    else:
        raise ValueError("Invalid input: Must be an integer or a DataFrame/Series with 'TIME' column.")


def plot_acclass_by_time_and_injury(df, acclass_map, injury_map):

    df["TIME_BIN"] = bintime_to_timeofday(df["TIME"])
    ...
    # Convert percentages to strings and append the % symbol for annotations
...
def plot_roc_curve(model, X_test, y_test, model_name, auc_scores=None):
    """
...
def timebin_to_numerical(input_data):
    """
    Converts time-of-day categories into numerical values. Handles both categorical strings and DataFrames.

    Parameters:
        input_data (str or pd.Series or pd.DataFrame): Time-bin category or a column/DataFrame with "TIME_BIN".

    Returns:
        int or pd.Series: Numeric value (e.g., 0 for Morning) for single categories,
                          or a DataFrame/Series with the "TIME_BIN_NUM" column added for DataFrames.
    """
    time_bin_mapping = {"Morning": 0, "Afternoon": 1, "Evening": 2, "Night": 3}

    # Handle single value for API use
    if isinstance(input_data, str):
        return time_bin_mapping.get(input_data, -1)

    # Handle DataFrame or Series for project use
    if isinstance(input_data, pd.Series):
        return input_data.map(time_bin_mapping)
    elif isinstance(input_data, pd.DataFrame) and "TIME_BIN" in input_data.columns:
        input_data["TIME_BIN_NUM"] = input_data["TIME_BIN"].map(time_bin_mapping)
        return input_data
    else:
        raise ValueError("Invalid input: Must be a string or a DataFrame/Series with 'TIME_BIN' column.")
...
def preprocess_datetime(datetime_str):
    ...
    try:
        ...
        # Use existing functions to get TIME_BIN and TIME_BIN_NUM
...
def check_correlation(data, target_column="ACCLASS"):
    ...
    # Also TIME_BIN would require converting into dummy variable or embedding, adding more complexity
    # Adding TIME_BIN_NUM the time of day will be already categorized into ordinal scale, making it computantionally
    # speaking faster to process for models like Random Forest and XGBoost.
    data["TIME_BIN_NUM"] = timebin_to_numerical(data["TIME_BIN"])
...
if __name__ == "__main__":
    ...
    rf_auc_scores = evaluate_auc(rf_model, X_test, y_test)
    # Plot ROC curve for Random Forest
    plot_roc_curve(rf_model, X_test, y_test, "Random Forest", rf_auc_scores)
    ...
    xgb_auc_scores = evaluate_auc(xgb_model, X_test, y_test)
    # Plot ROC curve for XGBoost
    plot_roc_curve(xgb_model, X_test, y_test, "XGBoost", xgb_auc_scores)
